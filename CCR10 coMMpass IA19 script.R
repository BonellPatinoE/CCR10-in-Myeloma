...
x<-str_extract(names(temp), "_[0-9]_")[1:80] 
x<-str_extract(x, "[0-9]")[1:80]
x

temp_bar <- data.frame(y = as.numeric(temp[temp$GENE == "CCR10", 1:203]), 
                       x = x,                
                       stringsAsFactors = F)
...
# PLOT variance of CCR10 expression across every sample
temp_bar%>%ggplot(aes(x = x, y = y)) + 
  ...
  ggtitle(label = "CCR10", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3)))+
...
temp%>%filter(GENE == "CCR10")
names(temp)
temp[temp$GENE == "CCR10", 1:162]
x<-str_extract(names(temp), "_[0-9]_")[1:162] 
x<-str_extract(x, "[0-9]")[1:162]
x

temp_bar <- data.frame(y = as.numeric(temp[temp$GENE == "CCR10", 1:162]), 
                       x = x,                
                       stringsAsFactors = F)
temp_bar
...
temp_bar1%>%ggplot(aes(x = x, y = y)) + 
  ...
  ggtitle(label = "CCR10", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_flip<-temp%>%filter(GENE == "CCR10")
head(temp_flip)
...
temp_bar2%>%ggplot(aes(ISS, temp_flip)) + 
  ...
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on ISS stage")+
  ggpubr::stat_pvalue_manual(stat_pvalue, label = "p.signif") +
  theme_minimal()
...
translocations_df<-data.frame(`611e8d8206f8c500013f9464_SeqFISH.Files_MMRF_CoMMpass_IA19_genome_tumor_only_mm_igtx_pairoscope`)
...
seqFISH<-data.frame(`611e8d8206f8c500013f9464_SeqFISH.Files_MMRF_CoMMpass_IA19_genome_gatk_cna_seqFISH`)
...
temp_bar2%>%ggplot(aes( x = NDS2, y = temp_flip)) + 
  ...
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on t(4:14) [NDS2] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = CCND1, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(11:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on t(11:14) [CCND1] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"],3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"],3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = CCND3, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(6:14) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on t(6:14) [CCND3] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "MYC mutation",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on MYC mutation", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MAF, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;16) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on t(14;16) [c-MAF] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MAFB, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "t(14;20) Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on t(14;20) [MAFB] status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = del17p13, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del17p13 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on del17p13 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = TP53, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "TP53 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on TP53 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = Gain1q21, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "Gain1q21 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on Gain1q21 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = del1p22, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "del1p22 Status",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on del1p22 status", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
temp_bar2%>%ggplot(aes( x = MYC, y = temp_flip)) + 
  ...
  scale_x_discrete(name = "MYC Status by seqFISH",
                   ...
                   labels = c("Negative", "Positive")) +
  scale_y_continuous(name = expression("Normalized log "[2]*" (Counts)")) +
  ggtitle(label = "CCR10 expression based on MYC status (SeqFISH)", 
          subtitle = paste0("log2FC = ",
                            round(res$log2FoldChange[res$GENE == "CCR10"], 3),
                            "   P-value = ", round(res$pvalue[res$GENE == "CCR10"], 3))) +
  theme_minimal()
...
################# Survival curve based on CCR10 expression ###################

survival<-data.frame(MMRF_CoMMpass_IA19_STAND_ALONE_SURVIVAL)
survival
survival1<-data.frame("ID" = survival$PUBLIC_ID,
                      "Time to death" = survival$deathdy)

survival1

survival1%>%filter(is.na (Time.to.death))
survival1%>%filter(!is.na (Time.to.death))

survival2<-survival1
survival2<-survival2%>%mutate(Alive = case_when(is.na(Time.to.death)~0, !is.na(Time.to.death)~1))
survival2
